...
class Checker:
    ...
    def verify(self, node: IRNode, cert: Certificate) -> CheckResult:
        if cert.subject_hash != node.hash:
            return CheckResult(
                Verdict.REJECT,
                reason="subject hash mismatch; certificate is stale or bound to another node",
                rule=cert.rule,
        ...
        if missing:
            return CheckResult(
                Verdict.UNKNOWN,
                reason=f"missing assumptions: {missing}",
                rule=cert.rule,
                certificate_id=cert.id,
            )
        ...
        if rule is None:
            return CheckResult(
                Verdict.REJECT, reason=f"unknown rule: {cert.rule}",
                rule=cert.rule, certificate_id=cert.id
            )
