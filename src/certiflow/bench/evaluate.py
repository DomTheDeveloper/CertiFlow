...
from .faults import mutate_group_grain,mutate_join_key,mutate_projection
@dataclass
...
def run_fault_evaluation(trials=100):
    def jc(i):
        c=Checker(); c.store.add(Fact.make("Key","customers",{"columns":("customer_id",)})); n=IRNode("Join",f"join_{i}",{"join_type":"left","equi":(("customer_id","customer_id"),)},("orders","customers")); cert=Certificate(n.hash,"join_fanout",witness={"left_col":"customer_id","right_col":"customer_id","right_relation":"customers"}); return c,n,cert,mutate_join_key
    def gc(i):
    ...
    return [_evaluate("join-key",trials,jc),_evaluate("aggregation-grain",trials,gc),_evaluate("projection-key",trials,pc)]
