...
def synthetic_pipeline(size=100, seed=7):
    if size < 3: raise ValueError("size must be >= 3")
    rng=random.Random(seed)
    nodes=[scan("customers",{"customer_id":"int","region":"text"}),scan("orders",{"order_id":"int","customer_id":"int","amount":"decimal"})]
    facts=[Fact.make("Key","customers",{"columns":("customer_id",)}),Fact.make("Key","orders",{"columns":("order_id",)})]
    current="orders"
